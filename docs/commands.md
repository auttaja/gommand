...
- `ArgTransformers`: This is an array of the `gommand.ArgTransformer` type. Each object in this array contains the following attributes:
    ...
    - `Default`: The value the argument will have if the user does not provide another argument (not in the case of an error from the argument transformer), this can either be a single value or a slice of values (for if `Greedy` is also set for example). Note that similarly to `Optional`, this either has to be at the end of the argument list of be followed by other `Optional` or `Default` arguments.
